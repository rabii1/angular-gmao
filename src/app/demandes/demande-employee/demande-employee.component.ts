...
export class DemandeEmployeeComponent implements OnInit {
  ...
  tableSizes = [ 6, 9, 12];
  id:any;
  constructor(private headerTitleService: HeaderTitleService,private sessionSotragesevice:SessionstorageService,private Serviceapibackend: BackendApiService,) { }

  ngOnInit(): void {
    this.headerTitleService.setTitle('  Demande traiter ');console.log('Suivi demandes ')
  ...
  loadDemandeintervention(){
    this.Serviceapibackend.getDemandeinterventionsByEmployee(this.id).subscribe((data:{}) => {
